...
def index():
    ...
    <head>
        ...
        <style>
            ...
            .card { background: #1e2025; padding: 40px; border-radius: 20px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); text-align: center; max-width: 600px; width: 100%; border: 1px solid #33353a; }
            h1 { color: #8aebff; margin-bottom: 8px; font-size: 28px; }
            p { color: #bbc9cd; margin-bottom: 40px; font-size: 16px; }
            ...
            .count { margin-top: 32px; color: #888; font-size: 14px; font-weight: 500; }
    ...
    <body>
        <div class="card">
            ...
            <div class="count">Labeled: {{ count }} / {{ total }}</div>
        </div>
        ...
        <script>
            function label(category) {
                fetch('/label', {
                    ...
                    body: JSON.stringify({ title: '{{ song.Title }}', category: category })
